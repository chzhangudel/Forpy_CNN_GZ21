...
def load_paper_net(device: str = 'gpu'):
    ...
    model_file = '/scratch/cimes/cz3321/MOM6/MOM6-examples/src/MOM6/config_src/external/ML_Forpy/Forpy_CNN_GZ21/trained_model_masks.pth'
    # ---------------------------------------------------- #
...
#    start_time = time.time()
   ...
   v = uv[1,:,:,:]*10.0
   dim = np.shape(u)
#    mask = np.zeros((dim[0],dim[1],dim[2]))
   mask = np.ones((dim[0],dim[1],dim[2]))
# #    print(index)
#    if index[0]==1:
#        mask[:,:10,:]=0
#    if index[1]==88:
...
#    if index[3]==80:
#        mask[-10:,:,:]=0
#     #    np.savetxt(f'mask{pe}.txt',(mask[:,:,1]))
   x = np.array([np.squeeze(u),np.squeeze(v),mask])
   if x.ndim==3:
     x = x[:,:,:,np.newaxis]
...
#   start_time = time.time()
  x = np.zeros((1, 3, 24, 24)).astype(np.float32)
#   x[:,1,:12, :] = 1.0*10
#   print(x[0,1,:,:])
